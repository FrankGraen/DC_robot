...
class gripper(am.arm):
    ...
    d = 10  # 内部齿轮直径，单位 mm
    max_width = 15.0  # task1 实测安全上限；负角张开，0°闭合
    ...
    def grasp(self, wideth=10, speed=10, force=3):
        ...
        '''
        if wideth < 0:
            raise ValueError(f"夹爪开合宽度不能为负，当前 wideth={wideth}")
        if wideth > self.max_width:
            print(f"请输入正确的开合宽度：0~{self.max_width:g}，已将 wideth 设置为 {self.max_width:g}")
            wideth = self.max_width
        ...
        if force > 120:
            ...
            force = 50
        angle = -wideth * 180 / (math.pi * self.d)  # 0°闭合，负角张开
        w = speed / (self.d / 2) / (math.pi * 2) * 60  # 将手爪开合速度转换为关节转速 r/min
        torque = force * (self.d / 2 / 1000)  # 手爪开合里转换成关节力矩 Nm
        print(f"[gripper] id={self.id_num}, width={wideth:.2f}mm, angle={angle:.2f}deg")
