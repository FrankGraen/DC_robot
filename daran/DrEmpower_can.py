...
class DrEmpower_can(object):
    ...
    def __init__(self, com='', uart_baudrate=115200):
        ...
        Args:
            ...
            uart_bautrate: 串口波特率；
            platform: 平台编号，
                      1代表 Windows，2代表在树莓派（raspbian），3代表 jetson nano（ubuntu），4代表苹果电脑 mac
        Returns:
            无。
        Raises:
            无。
        ...
        try:
            if len(id_list) == len(angle_list):
                if mode == 0:
                    ...
                    if len(self.cur_angle_list) != len(angle_list) or self.set_angle_mode_1_flag == 0:
                        self.cur_angle_list = []
                        for i in range(len(id_list)):
    ...
    def read_property(self, id_num=0, property=''):
        ...
        Args：
              id_num：一体化关节 ID 编号，如果不知道当前一体化关节 ID 编号，可以用 0广播。
                      但此时如果总线上有多个一体化关节，会造成总线通信干扰，不可使用 0 广播。
            property：需要设置的属性参数名称，例如 "vbus_voltage"，"axis.config.can_id"等，
                      具体参数名称见 interface_enums.py 文件里 property_address 字典里的键值。
        Returns：
            value：返回对应属性参数的值。
    ...
    def set_pid(self, id_num=0, P=20, I=25, D=20):
        ...
        如用户决定永久使用某组 PID 则可在使用该函数设置 PID 后，请紧接着使用 self.save_config 函数。
    ...
    def preset_speed(self, id_num=0, speed=10, param=0, mode=1):
        ...
        Args:
            ...
            mode:   控制模式选择
                    ...
                    mode!=0,转速爬升控制模式，一体化关节将按照目标加转速axis.controller.config_.speed_ramp_rate变化到speed。

        Note:
            在转速爬升模式下，如果目标加转速axis.controller.config_.speed_ramp_rate设置为0，则一体化关节转速将保持当前值不变。
    ...
    def preset_torque(self, id_num=0, torque=0.1, param=0, mode=1):
        ...
        Args:
            id_num: 需要设置的一体化关节ID编号,如果不知道当前一体化关节ID，可以用0广播，如果总线上有多个一体化关节，则多个一体化关节都会执行该操作。
            torque: 一体化关节输出（Nm)
            param: mode=0,改参数无意义；mode!=0,力矩上升转速axis.controller.config.torque_ramp_rate（Nm/s）
            mode:   控制模式选择
                    mode=0, 力矩直接控制模式，一体化关节将目标力矩直接设为torque
                    mode!=0,力矩爬升控制模式，一体化关节将按照力矩上升速率axis.controller.config.torque_ramp_rate（Nm/s）变化到torque。

        Note;
            如果一体化关节转速超过您设置的 speed_limit ，一体化关节输出的力矩将会减小。
            可以设置 axis.controller.config.enable_current_mode_speed_limit = False 来禁止力矩减小。
            另外在力矩爬升控制模式下，如果点击力矩上升速率axis.controller.config.torque_ramp_rate为0，则点击力矩将在当前值保持不变。
