...
实验采用标准 DH 参数建立六自由度机械臂模型。模型在 \texttt{daran/task1/dh\_cartesian\_line\_motion.ipynb}、\texttt{daran/task2/pick\_and\_place.py} 和 \texttt{daran/task3/visual\_grasp.py} 中保持一致，均由 \texttt{roboticstoolbox} 的 \texttt{DHRobot} 和 \texttt{RevoluteDH} 构造。
...
\]
在 notebook 中，起点由正运动学得到，终点通过 \texttt{path\_end\_mm} 指定。程序调用 \texttt{rtb.tools.trajectory.ctraj} 在起止末端位姿之间生成笛卡尔路径，再对每个路径点使用 \texttt{ikine\_LM} 求逆运动学。为了保证关节空间轨迹连续，第 $k$ 个点的逆解会作为第 $k+1$ 个点的初值。
